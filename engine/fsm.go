...
import (
	...
	"github.com/alwaysthanks/xcron/lib/routine"
...
var (
	//parallel goroutine
	routinePool = routine.NewRoutinePool(100)
)
...
func (f *fsm) Apply(l *raft.Log) interface{} {
	...
	case "task":
		routinePool.Start(context.Background(), func(t *routine.Task) error {
			if ret := f.applyTask(dsc.Value); ret != nil {
				f.logger.Printf("[error][fsm.Apply] applyTask key:%s, ret: %s", dsc.Key, ret)
			}
			return nil
		})
		return nil
